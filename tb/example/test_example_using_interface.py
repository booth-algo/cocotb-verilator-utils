...
@cocotb.test()
async def test_interface_back_to_back_requests(dut):
    """Test back-to-back requests through interface (with small gap)"""
    ...
    # Send back-to-back requests with minimal gap
    test_values = [0xFEEDFACE, 0xDEADC0DE]

    dut._log.info("Testing back-to-back interface requests")

    # Send first request
    dut.m2s_request.value = test_values[0]
    dut.m2s_request_valid.value = 1
    await RisingEdge(dut.clk)

    # Clear valid for one cycle (proper pulse)
    dut.m2s_request_valid.value = 0
    await RisingEdge(dut.clk)

    # Immediately send second request
    dut.m2s_request.value = test_values[1]
    dut.m2s_request_valid.value = 1
    await RisingEdge(dut.clk)
    ...
    for cycle in range(max_wait_cycles):
        ...
        if dut.s2m_response_valid.value == 1:
            ...
            dut._log.info(f"Interface back-to-back response {len(responses_received)}: 0x{response_data:08X}")

            # Stop after we get expected number of responses
            if len(responses_received) >= len(test_values):
                break

    # Verify we got the expected number of responses
    assert len(responses_received) == len(test_values), f"Expected {len(test_values)} responses, got {len(responses_received)}"

    # Verify that all sent values were received (in any order)
    for expected_value in test_values:
        assert expected_value in responses_received, f"Expected value 0x{expected_value:08X} not found in responses"

    dut._log.info("✓ Interface back-to-back test passed!")
